...
class GraphAnalysis:
    ...
    def mainMethod(self, sourceNodeName, whichMethod, findAllPathsDic, validaPathsDic):
        ...
        if (whichMethod):
            paths = self.findAllPaths(sourceNodeName=sourceNodeName, cases=cases, graphName=graphName,
                                      relationShip=relationship, k=k, targetNodeName=targetNodeName)
        else:
            paths = self.findAllPathsViseVerse(sourceNodeName=sourceNodeName, nodesTable=nodesTable,
                                               graphName=graphName, nodeNames=nodesName, edgesNames=edgesName, k=k,
                                               TargetType=targetType)
        if paths.empty:
            print("there are no paths to be validated")
            return
        finalOutput = self.validatePath(paths=paths, sourceNodeName=sourceNodeName, nodeNames=nodesName,
                                        edgesNames=edgesName, nodesTable=nodesTable, theDesiredType=thedesiredType)
    ...
    def findAllPathsViseVerse(self, SourceNodeName, nodesTable, graphName, nodeNames, edgesNames, cases, k=1,
                              TargetType=""):
        ...
        for node in range(len(filteredNodesTable)):
            ...
            out = self.findAllPaths(sourceNodeName=souceNode, sourceLabel=TargetLabel, cases=cases, graphName=graphName,
                                    k=k, targetNodeName=SourceNodeName, targetLabel=SourceLabel)
    ...
    def pathsValidation(self, pathsWithCorrectTargetNodes, nodeNames, edgesNames):
        ...
        for path in range(len(pathsWithCorrectTargetNodes)):
            ...
            for element in range(len(nodeNamesColumn[path]) - 1):
                ...
                if ((previousNode in nodeNames) & (currentNode in edgesNames)) | (
                        (previousNode in edgesNames) & (currentNode in nodeNames)):
                    validUntilNow = True
                else:
                    validUntilNow = False
                    break
